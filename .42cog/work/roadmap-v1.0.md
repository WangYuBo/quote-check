...
**m3 代码闭环（2026-04-25）**：MAS-5 CSV/Word 双格式导出（版本戳 + 三维度独立列）✓ · MAS-6 TTL销毁 cron + Pino logger ✓ · 观测接入 /api/admin/health ✓ + /api/admin/audit 查询 API ✓ · MS-L-11 协议签署弹窗 ✓ · E2E Playwright 骨架（L-01/02/03/07/10）✓ · 决策门 DG-m2.3/DG-m3.1/DG-m3.2 全部记录。待完成：Vercel production 部署 + Neon main 迁移 + Inngest Cloud PUT /sync。
...
- **DG-m2.3（MAS-3 末期）**：是否启用 Better Auth `requireEmailVerification=true`？
  ...
  - **DG-m2.3 决策（2026-04-25）**：v1.0 保持 `requireEmailVerification=false`（C 端冷启动优先降摩擦）；v1.1 升级为 true 并配 Better Auth hook 分角色策略。依据：beta 阶段用户基数极小（<50人），错误配置的风险低于流失风险。
...
| 项 | 交付判据 | 状态 |
|----|---------|------|
| MAS-5 版本冻结 | frozen_at 写入后 T-01/T-02/T-03 触发器实战验证（契约测 → 真实数据）；report_snapshot 导出 Word（`docx` 库）+ CSV | ✓ CSV+Word 导出 API |
| MAS-6 保密闭环 | TTL 销毁 Inngest cron（每 10 min 扫 task.ttl_expires_at）+ Pino redact 生产配置（raw text 不落日志）+ Vercel Blob del 脚本 | ✓ TTL+Pino 实装 |
| 观测接入 | /api/admin/health（Neon 连通性）+ audit_log 查询 API；DG-m3.1 决策跳过 Sentry | ✓ 两端点均实装 |
| MS-L-11 协议签署 | 签署弹窗 + user_agreement_acceptance append-only 写入（T-05 触发器兜底）+ 拒绝签署 → signOut | ✓ AgreementModal 实装 |
| E2E smoke | Playwright 覆盖 L-01/02/03/07/10 五条主流程；CI 中运行 | ✓ 骨架实装（需 dev server 验证） |
| 上线部署 | Vercel production 部署 + Neon main branch 迁移 + Inngest Cloud PUT /sync | ⏳ 待执行 |
...
- **DG-m3.1（观测接入前）**：是否接入 Sentry？
  ...
  - **DG-m3.1 决策（2026-04-25）**：v1.0 跳过 Sentry，改用 Pino 结构化日志 + Vercel Log Drain（已实装 lib/logger.ts）；原因：PII scrubber 配置复杂度 > beta 阶段 bug 诊断收益，5K events/month 配额在上线后首周就会耗尽。v1.1 再按需接入。
- **DG-m3.2（上线前）**：public 还是 invite-only？
  - 背景：real.md 未规定；notes #1 提示信任建立需要缓释
  - 建议：invite-only 软启动 + B 端付费邀请码（同时验证付费意愿）
  - **DG-m3.2 决策（2026-04-25）**：v1.0 public（注册即可用），不设邀请码门槛；依据：协议签署（MS-L-11）+ 成本守卫（cost-guard）已构成足够的滥用防线；邀请码机制推迟至用户规模超过可控范围时再加。
